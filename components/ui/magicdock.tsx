...
function DockItem({
  ...
  return (
    ...
    >
      <motion.div
        className={cn(
          "relative flex h-full w-full items-center justify-center rounded-full  border-2 shadow-md transition-colors duration-300",
          getBorderStyles()
        )}
        initial={{}}
      >
        {item.image ? (
          ...
          <div className="flex items-center justify-center text-gray-800">{item.icon}</div>
        )}
      </motion.div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          `absolute bottom-2  left-1/2 transform -translate-x-1/2 flex items-end  gap-4 rounded-2xl border-slate-700/50 border-2 pb-2 px-4 ${getBgStyles()}`,
          className
        )}
