...
import LustreText from "./lustretext";
...
const TestimonialCard: React.FC<TestimonialCardProps> = React.memo(
  ({ testimonial, index, cardClassName = "" }) => {
...
const KineticTestimonial: React.FC<KineticTestimonialProps> = ({
  ...
  const renderColumn = useCallback(
    (columnTestimonials: TestimonialWithId[], colIndex: number, prefix: string, containerHeight: number) => {
      ...
      return (
        <div
          key={`${prefix}-${colIndex}`}
          className="flex-1 relative testimonial-column"
  ...
  return (
    <section className={`${className}`}>
      <div className=" text-gray-900 dark:text-white py-8 pb-3 flex flex-col items-center overflow-hidden px-4 md:px-6">
        <h2 className="mb-10 text-5xl md:text-6xl font-extrabold bg-gradient-to-r from-gray-400 via-gray-500 to-gray-500 bg-clip-text text-transparent leading-tight drop-shadow-lg">
          <LustreText text={title} />
        </h2>
