...
export function toast(messageOrOptions: string | (ToastOptions & { title: string }), options?: ToastOptions): string {
...
const ToastIcons = {
  success: <CheckCircle className="w-5 h-5 text-green-600 dark:text-green-400 flex-shrink-0" />,
  destructive: <AlertCircle className="w-5 h-5 text-red-600 dark:text-red-400 flex-shrink-0" />,
  warning: <AlertCircle className="w-5 h-5 text-yellow-600 dark:text-yellow-400 flex-shrink-0" />,
...
const ToastComponent: React.FC<ToastProps> = ({
  ...
  const renderAction = () => {
    ...
    if (React.isValidElement(action)) {
      const actionElement = action as React.ReactElement<{ onClick?: (e: React.MouseEvent) => void }>;
      return (
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-3 w-full min-w-0">
        {variant !== "default" && ToastIcons[variant as keyof typeof ToastIcons]}
        <div className="flex-1 min-w-0">
          {title && <div className="font-semibold text-sm truncate">{title}</div>}
          {description && (
            <div className="text-xs opacity-70 truncate mt-1">{description}</div>
          )}
        </div>
      </div>
...
const ToastStack: React.FC<ToastStackProps> = ({ toasts, position, onRemoveToast }) => {
  ...
  const handleRemoveToast = useCallback((id: string) => {
    ...
    if (toastToRemove && toasts.filter((t: ToastState) => t.position === toastToRemove.position).length === 1) {
      setIsHovered(false);
      setIsTapped(false);
    }
    onRemoveToast(id);
